...
def unitBuiltFeats(pCity, pUnit):
    ...
    if featPopup(pCity.getOwner()):
        ...
        if not gc.getPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_GUN):
            if pUnit.getUnitType() == gc.getInfoTypeForString("UNIT_TRADE_MERCHANT") or pUnit.getUnitType() == gc.getInfoTypeForString("UNIT_CARAVAN"):
                ...
                if 1 == 1:
                    ...
                    popupInfo.setData2(pCity.getID())
                    popupInfo.setText(localText.getText("TXT_KEY_FEAT_TRADE_MERCHANT", (pUnit.getNameKey(), pCity.getNameKey(), )))
                    popupInfo.setOnClickedPythonCallback("featAccomplishedOnClickedCallback")
                    popupInfo.setOnFocusPythonCallback("featAccomplishedOnFocusCallback")
                    popupInfo.addPythonButton(localText.getText("TXT_KEY_FEAT_ACCOMPLISHED_OK", ()), "")
                    popupInfo.addPopup(pCity.getOwner())
        ...
        # Getreidekarren / Supply food
        if not gc.getPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_ARMOR):
            if pUnit.getUnitType() == gc.getInfoTypeForString("UNIT_SUPPLY_FOOD"):
                ...
                if 1 == 1:
                    ...
                    popupInfo.setData2(pCity.getID())
                    popupInfo.setText(localText.getText("TXT_KEY_FEAT_SUPPLY_FOOD", (pUnit.getNameKey(), pCity.getNameKey(), )))
                    popupInfo.setOnClickedPythonCallback("featAccomplishedOnClickedCallback")
                    popupInfo.setOnFocusPythonCallback("featAccomplishedOnFocusCallback")
                    popupInfo.addPythonButton(localText.getText("TXT_KEY_FEAT_ACCOMPLISHED_OK", ()), "")
                    popupInfo.addPopup(pCity.getOwner())
