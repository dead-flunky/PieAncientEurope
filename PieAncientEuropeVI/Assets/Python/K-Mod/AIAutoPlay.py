    ...
    def __init__(self, customEM):
        ...
        if self.blockPopups:
            ...
            try:
                ...
                CvUtil.pyPrint("Failed to remove 'onBeginGameTurn', perhaps not registered")
                self.customEM.setEventHandler("BeginGameTurn", self.onBeginGameTurn)


    def removeEventHandlers(self) :
        CvUtil.pyPrint("Removing event handlers from AIAutoPlay")
...
#            CvTopCivs.CvTopCivs().turnChecker(iGameTurn)
    ...
    def AIChooserHandler(self, playerID, netUserData, popupReturn):
        ...
        if numTurns > 0:
            if self.LOG_DEBUG:
                CyInterface().addImmediateMessage("Fully automating for %d turns"%(numTurns),"")
