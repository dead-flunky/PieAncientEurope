...
## AI AutoPlay ##
    ...
    def onModNetMessage(self, argsList):
        ...
        # 677 Goldkarren / Beutegold / Treasure zw 10 und 60
        elif iData1 == 677:
            pPlayer = gc.getPlayer(iData4)
            pUnit = pPlayer.getUnit(iData5)
            if iData2 == 1:
...
# Vasallen Technologie +++++++++++++++++++++++++
        ...
        # Stall: Elefant, Kamel, Pferd
            ...
            pUnit = pPlayer.getUnit(iData5)
            eBonus = -1
            if   iData3 == 1: eBonus = gc.getInfoTypeForString("BONUS_IVORY")
            elif iData3 == 2: eBonus = gc.getInfoTypeForString("BONUS_CAMEL")
            elif iData3 == 3: eBonus = gc.getInfoTypeForString("BONUS_HORSE")
            if eBonus != -1:
        ...
        # Reservist -> Veteran
            ...
            if iUnit1 == -1: iUnit1 = gc.getInfoTypeForString("UNIT_SCHILDTRAEGER")
            ...
            if iUnit2 == -1: iUnit2 = gc.getInfoTypeForString("UNIT_REFLEX_ARCHER")
            ...
            if iUnit3 == -1: iUnit3 = gc.getInfoTypeForString("UNIT_AUXILIAR")
        ...
        # iData3 = 2: UNIT_SUPPLY_WAGON Getreideaufnahme
            ...
            if iData3 == 1:
                ...
                if iStoredCityFood >= iDiff: iFoodChange = iDiff
                else: iFoodChange = iStoredCityFood
        ...
        # 752: bless units
            ...
            if iData2 == 1: 
                ...
                PAE_Unit.doGoToNextUnit(pUnit)
            else: PAE_Unit.doBlessUnits(pUnit)
        ...
        # Slave -> Latifundium oder Village/Dorf
            ...
            pUnit = pPlayer.getUnit(iData5)
            pPlot = pUnit.plot()
            if iData2 == 1: PAE_Sklaven.doUpgradeLatifundium(pPlot)
            else: pPlot.changeUpgradeProgress(10)
            pUnit.doCommand(CommandTypes.COMMAND_DELETE, 1, 1)
            pUnit = None
...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    def onGameStart(self, argsList):
        ...
        # Tiefsee setzen
        if sScenarioName == "": PAE_Turn_Features.doPlaceDeepOcean()
...
############################################
    ...
    def onEndPlayerTurn(self, argsList):
        ...
        if iGameTurn > 1: PAE_Barbaren.createCampUnit(iPlayer, iGameTurn)
        ...
        if not pWinnerTeam.isHasTech(iTech):
            if pWinnerPlayer.canResearch(iTech, False):
                ...
                if pWinnerPlayer.isHuman():
                    CyInterface().addMessage(iWinnerPlayer, True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_TECH_BY_UNIT", (gc.getTechInfo(iTech).getDescription(), iCost)), None, 2, None, ColorTypes(8), 0, 0, False, False)
        ...
        if iImprovement > -1 and not bWinnerAnimal and not bLoserAnimal and pLoser.isMilitaryHappiness():
            if iImprovement != gc.getInfoTypeForString("IMPROVEMENT_CITY_RUINS") and iImprovement != gc.getInfoTypeForString("IMPROVEMENT_GOODY_HUT"):
                ...
                if CvUtil.myRandom(100, "IMPROVEMENT_DESTROYED_COMBAT") < iChance:
                    ...
                    if pWinnerPlayer.isHuman():
                        CyInterface().addMessage(iWinnerPlayer, True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_IMPROVEMENT_DESTROYED_COMBAT", (gc.getImprovementInfo(iImprovement).getDescription(),)), "AS2D_DESTROY", 2, gc.getImprovementInfo(iImprovement).getButton(), ColorTypes(13), pLoserPlot.getX(), pLoserPlot.getY(), True, True)
                    if pLoserPlayer.isHuman():
                        CyInterface().addMessage(iLoserPlayer, True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_IMPROVEMENT_DESTROYED_COMBAT", (gc.getImprovementInfo(iImprovement).getDescription(),)), "AS2D_DESTROY", 2, gc.getImprovementInfo(iImprovement).getButton(), ColorTypes(13), pLoserPlot.getX(), pLoserPlot.getY(), True, True)
                    ...
                    if iPlotOwner != -1 and iPlotOwner != iWinnerPlayer and iPlotOwner != iLoserPlayer:
                        if gc.getPlayer(iPlotOwner).isHuman():
                            CyInterface().addMessage(iPlotOwner, True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_IMPROVEMENT_DESTROYED_COMBAT_PLOT_OWNER", (pWinnerPlayer.getName(), pLoserPlayer.getName(), gc.getImprovementInfo(iImprovement).getDescription())), "AS2D_DESTROY", 2, gc.getImprovementInfo(iImprovement).getButton(), ColorTypes(13), pLoserPlot.getX(), pLoserPlot.getY(), True, True)
            ...
            if bUnitFlucht and pLoserFlucht is not None:
                ...
                if not bWinnerIsDead:
                    ...
                    if bLoserAnimal:
                        ...
                        if iLoserUnitType not in L.LUnitWarAnimals: PAE_Unit.huntingResult(pLoser, pWinner)
                    else:
            ...
            if pUnit.canMove():
                ...
                if CvUtil.myRandom(100, "Handelsstrasse bauen") < 2:
                    ...
                    if pPlotTradeRoad is not None:
                        ...
                        if pPlayer.isHuman():
                            CyInterface().addMessage(eOwner, True, 10, CyTranslator().getText("TXT_KEY_TRADE_ROUTE_BUILT", (pPlayer.getName(), pPlayer.getCivilizationShortDescriptionKey(), pCity.getName(), pSourceCity.getName())), "AS2D_WELOVEKING", 2, "Art/Terrain/Routes/handelsstrasse/button_handelsstrasse.dds", ColorTypes(10), pPlotTradeRoad.getX(), pPlotTradeRoad.getY(), True, True)
        ...
        # Nur 1x Fernangriff danach nur bewegen => GlobalDefines RANGED_ATTACKS_USE_MOVES=0
            ...
            for i in listUnitIds:
                ...
                if pLoopUnit.isRanged() and pLoopUnit.canMove():
                    # Liste fuer Kosten
                    if pLoopUnit.canAttack(): lUnits.append(pLoopUnit)
            ...
            if pPlayer.isHuman() and lUnits:
                if pPlayer.getCivilizationType() not in L.LFernangriffNoCosts:
                    ...
                    for unit in lUnits:
                        ...
                        if iUnitClass in L.DFernangriffCosts: iGold += L.DFernangriffCosts[iUnitClass]
                        elif iUnitType in L.DFernangriffCosts: iGold += L.DFernangriffCosts[iUnitType]
                        elif iUnitCombat in L.DFernangriffCosts: iGold += L.DFernangriffCosts[iUnitCombat]
                        else: iGold += 1
    ...
    def onUnitMove(self, argsList):
        ...
        if pUnit.isBarbarian():
            ...
            if PAE_Barbaren.doOnUnitMove(pUnit, pPlot, pOldPlot): return
                ...
                if CvUtil.myRandom(10, "pillage_promo") < 1:
                    for iPromo in L.LPromoPillage:
                        if not pUnit.isHasPromotion(iPromo):
                            pUnit.setHasPromotion(iPromo, True)
                            if gc.getPlayer(iPlayer).isHuman():
                                CyInterface().addMessage(iPlayer, True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_UNIT_GETS_PROMOTION", (pUnit.getName(), gc.getPromotionInfo(iPromo).getDescription())), "AS2D_IF_LEVELUP", 2, gc.getPromotionInfo(iPromo).getButton(), ColorTypes(13), iPlotX, iPlotY, True, True)
                            break
    ...
    def onTechSelected(self, argsList):
        ...
        if not self.__LOG_TECH:
            return
        CvUtil.pyPrint('%s was selected by Player %d' 
    ...
    def onCityRazed(self, argsList):
		...
		#	if not pOwner.isBarbarian() and pOwner.getNumCities() > 0:
		#		if gc.getTeam(pOwner.getTeam()).isAtWar(gc.getPlayer(iPlayer).getTeam()):
		#			if gc.getNumEventTriggerInfos() > 0: # prevents mods that don't have events from getting an error
		#				iEvent = CvUtil.findInfoTypeNum(gc.getEventTriggerInfo, gc.getNumEventTriggerInfos(),'EVENTTRIGGER_PARTISANS')
		#				if iEvent != -1 and gc.getGame().isEventActive(iEvent) and pOwner.getEventTriggerWeight(iEvent) < 0:
		#					triggerData = pOwner.initTriggeredData(iEvent, true, -1, city.getX(), city.getY(), iPlayer, city.getID(), -1, -1, -1, -1)
        ...
        # Seek Plots
            ...
            if rebelPlotArray:
                ...
                if iUnit2 == -1: iUnit2 = gc.getInfoTypeForString("UNIT_AXEMAN")
    ...
    def onCityAcquired(self, argsList):
        ...
        bAssimilation = False
        if not pPlayer.isBarbarian(): bAssimilation = gc.getTeam(pPlayer.getTeam()).isHasTech(gc.getInfoTypeForString("TECH_ASSIMILATION"))
        ...
        if gc.getTeam(pPreviousOwner.getTeam()).isAtWar(pPlayer.getTeam()):
            ...
            if not bAssimilation:
                ...
                if not bTrade: PAE_City.doDeportation(pCity, iNewOwner, iPreviousOwner)
                ...
                if iNewOwner != gc.getBARBARIAN_PLAYER():
                    ...
                    if not pTeam.isHasTech(gc.getInfoTypeForString("TECH_KRIEGERETHOS")):
                        if pTeam.isHasTech(gc.getInfoTypeForString("TECH_BUCHSTABEN")):
                            ...
                            if not pCity.isHasBuilding(iBuilding): pCity.setNumRealBuilding(iBuilding, 1)
    ...
    def onCityDoTurn(self, argsList):
        ...
        if iPlayer != gc.getBARBARIAN_PLAYER() and popCity > 5:
            if pPlayer.getCivilizationType() != gc.getInfoTypeForString("CIVILIZATION_CITYSTATE"):
