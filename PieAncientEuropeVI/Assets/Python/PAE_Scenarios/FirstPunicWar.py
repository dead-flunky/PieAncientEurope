...
def onEndPlayerTurn(iPlayer, iGameTurn):
    ...
    # Bedingung Syrakus mit Rom im Krieg
        ...
        if iPlayer == iCivSyrakus and pSyrakus.isHuman() and gc.getTeam(iCivSyrakus).isAtWar(iCivRome):
            ...
            for i in LNewUnits:
                for _ in range(3):
                    pUnit = pSyrakus.initUnit(i, pPlot.getX(), pPlot.getY(), UnitAITypes.UNITAI_ATTACK,
                                              DirectionTypes.DIRECTION_SOUTH)
                    pUnit.setExperience(2, -1)

            pUnit = pSyrakus.initUnit(gc.getInfoTypeForString("UNIT_SUPPLY_WAGON"), pPlot.getX(),
                                      pPlot.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
    ...
    # HI Syrakus
        ...
        if iPlayer == iCivSyrakus and pSyrakus.isHuman():
            ...
            for i in LNewUnits:
                pUnit = pSyrakus.initUnit(i, Landungsplot.getX(), Landungsplot.getY(),
                                          UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
                pUnit.setExperience(2, -1)

            # Schiffe erstellen
            for _ in range(3):
                pUnit = pSyrakus.initUnit(gc.getInfoTypeForString("UNIT_TRIREME"),
                                          Schiffsplot.getX(), Schiffsplot.getY(), UnitAITypes.UNITAI_ASSAULT_SEA, DirectionTypes.DIRECTION_SOUTHWEST)
                pUnit.setExperience(2, -1)
    ...
    # Mit Hilfe von Enterbrücken und größeren Schiffsbesatzungen erringen die Römer einen spektakulären Seesieg am Kap Economus (256 BC)."
        ...
        if iPlayer == iCivRome and pPlot.getOwner() == iCivRome:
            ...
            for i in LNewUnits:
                for _ in range(4):
                    pUnit = pRome.initUnit(i, pPlot.getX(), pPlot.getY(), UnitAITypes.UNITAI_ASSAULT_SEA,
                                           DirectionTypes.DIRECTION_SOUTH)
                    pUnit.setExperience(2, -1)
                    pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_COMBAT1"), True)
    ...
    # Konsul Lucius Cornelius Scipio landet 259 BC auf Korsika und Sardinien. Er kann die Inseln jedoch nicht dauerhaft besetzen.
        ...
        if gc.getPlayer(iPlayer).isHuman():
            ...
            for i in LNewUnits:
                pUnit = pRome.initUnit(i, Landungsplot.getX(), Landungsplot.getY(),
                                       UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
                pUnit.setExperience(2, -1)

            # Schiffe erstellen
            for _ in range(2):
                pUnit = pRome.initUnit(gc.getInfoTypeForString("UNIT_QUADRIREME"),
                                       Schiffsplot.getX(), Schiffsplot.getY(), UnitAITypes.UNITAI_ASSAULT_SEA, DirectionTypes.DIRECTION_SOUTHWEST)
                pUnit.setExperience(2, -1)
                pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_COMBAT1"), True)
                pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_CORVUS1"), True)
            ...
            pUnit = pRome.initUnit(gc.getInfoTypeForString("UNIT_SUPPLY_WAGON"), Landungsplot.getX(),
                                   Landungsplot.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
            PAE_Unit.initSupply(pUnit)

            pUnit = pRome.initUnit(gc.getInfoTypeForString("UNIT_LEGION_TRIBUN"), Landungsplot.getX(),
                                   Landungsplot.getY(), UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
            pUnit.setExperience(2, -1)
            pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_COMBAT1"), True)
            ...
            # Schiffe erstellen
            for _ in range(5):
                pUnit = pRome.initUnit(gc.getInfoTypeForString("UNIT_QUADRIREME"), Schiffsplot.getX(),
                                       Schiffsplot.getY(), UnitAITypes.UNITAI_ASSAULT_SEA, DirectionTypes.DIRECTION_SOUTHWEST)
                pUnit.setExperience(2, -1)
                pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_COMBAT1"), True)
                pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_CORVUS1"), True)
    ...
    # Die Schwäche von Karthago ausnutzend, erheben sich die lokalen Numiderstämme zu einem Aufstand um die Vorherrschaft der Punier abzuschütteln.
        ...
        if gc.getPlayer(iPlayer).isHuman():
            ...
            for i in range(4):
                ...
                for i in LNewUnits:
                    pUnit = pBarbs.initUnit(i, pPlot.getX(), pPlot.getY(), UnitAITypes.UNITAI_ATTACK,
                                            DirectionTypes.DIRECTION_SOUTH)
                    pUnit.setExperience(2, -1)
    ...
    # Mit Hilfe des Söldnerführers Xanthippos gelingt es den Puniern die römische Invasion zurück zu schlagen (Schlacht von Tunis 255 BC).
        ...
        if gc.getPlayer(iPlayer).isHuman():
            ...
            for i in LNewUnits:
                for _ in range(2):
                    pUnit = pRome.initUnit(i, Schiffsplot.getX(), Schiffsplot.getY(),
                                           UnitAITypes.UNITAI_ASSAULT_SEA, DirectionTypes.DIRECTION_SOUTH)
                    pUnit.setExperience(2, -1)
                    pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_COMBAT1"), True)
                    pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_CORVUS1"), True)
    ...
    # Im Jahr 251 BC gelingt Karthago die Rückeroberung von Agrigent
        ...
        if pPlot.getOwner() == iCivRome and gc.getPlayer(iPlayer).isHuman():
            ...
            for i in LNewUnits:
                pUnit = pCarthage.initUnit(i, Landungsplot.getX(), Landungsplot.getY(),
                                           UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
                pUnit.setExperience(2, -1)

            pUnit = pCarthage.initUnit(gc.getInfoTypeForString("UNIT_SUPPLY_WAGON"), Landungsplot.getX(),
                                       Landungsplot.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
            PAE_Unit.initSupply(pUnit)
            ...
            for i in LNewUnits:
                for _ in range(2):
                    pUnit = pCarthage.initUnit(i, Schiffsplot.getX(), Schiffsplot.getY(), 
                                               UnitAITypes.UNITAI_ASSAULT_SEA, DirectionTypes.DIRECTION_SOUTH)
                    pUnit.setExperience(2, -1)
                    pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_COMBAT1"), True)
    ...
    # ermuntert aber wohlhabende Römer zu privaten Kaperfahrten vor den Küsten der Punier.
        ...
        if gc.getPlayer(iPlayer).isHuman():
            ...
            for p in lPlots:
                ...
                pUnit = pRome.initUnit(gc.getInfoTypeForString("UNIT_PIRAT_TRIREME"), p.getX(), p.getY(),
                                       UnitAITypes.UNITAI_ASSAULT_SEA, DirectionTypes.DIRECTION_SOUTH)
                pUnit.setExperience(2, -1)
                pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_COMBAT1"), True)
    ...
    # Nachdem der Krieg zu Land und zur See stagniert, unternimmt die punische Flotte Plünderungszüge an der italienischen Südküste.
        ...
        if pPlot.getOwner() == iCivRome and gc.getPlayer(iPlayer).isHuman():
            ...
            for p in lPlots:
                for i in range(p.getNumUnits()):
                    p.getUnit(i).jumpToNearestValidPlot()
                pShip = pCarthage.initUnit(gc.getInfoTypeForString("UNIT_TRIREME"), p.getX(), p.getY(),
                                           UnitAITypes.UNITAI_ASSAULT_SEA, DirectionTypes.DIRECTION_SOUTH)
                pShip.setExperience(2, -1)
                pUnit = pCarthage.initUnit(gc.getInfoTypeForString("UNIT_SCHILDTRAEGER_IBERIA"), p.getX(),
                                           p.getY(), UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
                pUnit.setExperience(2, -1)
                pUnit.setTransportUnit(pShip)
                pUnit = pCarthage.initUnit(gc.getInfoTypeForString("UNIT_SPEARMAN_CARTHAGE"), p.getX(),
                                           p.getY(), UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
                pUnit.setExperience(2, -1)
                pUnit.setTransportUnit(pShip)
                pUnit = pCarthage.initUnit(gc.getInfoTypeForString("UNIT_SKIRMISHER"), p.getX(),
                                           p.getY(), UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
                pUnit.setExperience(2, -1)
                pUnit.setTransportUnit(pShip)
    ...
    # Nach dem römischen Seesieg bei den Ägadischen Inseln (241 BC) muß Karthago um Frieden bitten.
        ...
        if pPlot.getOwner() == iCivRome:
            if iPlayer == iCivRome:
                ...
                for i in LNewUnits:
                    for _ in range(3):
                        pUnit = pCarthage.initUnit(i, pPlot.getX(), pPlot.getY(),
                                                   UnitAITypes.UNITAI_ASSAULT_SEA, DirectionTypes.DIRECTION_SOUTH)
                        pUnit.setExperience(2, -1)
                        pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_COMBAT1"), True)
                        pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_CORVUS1"), True)
        ...
        if pPlot.getOwner() == iCivRome and gc.getPlayer(iPlayer).isHuman():
            if pPlot.isCity():
                ...
                for _ in range(iNum):
                    ...
                    pUnit = pBarbs.initUnit(LNewUnits[iRand], pPlot.getX(), pPlot.getY(),
                                            UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
                    pUnit.setExperience(2, -1)
