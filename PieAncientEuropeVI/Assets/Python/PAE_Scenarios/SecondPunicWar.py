    ...
    if iGameTurn == 150:
        if iPlayer == iRome:
            ...
            if pPlot.getOwner() == iRome and gc.getTeam(gc.getPlayer(iRome)).isAtWar(gc.getPlayer(iCarthago)):
                ...
                pUnit = gc.getPlayer(iCarthago).initUnit(gc.getInfoTypeForString("UNIT_MOUNTED_SACRED_BAND_CARTHAGE"), Landungsplot.getX(), Landungsplot.getY(), UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
                pUnit.setExperience(2, -1)
                pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_COMBAT1"), True)
                pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_COMBAT2"), True)
