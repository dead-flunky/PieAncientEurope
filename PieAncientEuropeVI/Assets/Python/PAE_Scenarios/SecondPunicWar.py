...
def onEndPlayerTurn(iPlayer, iGameTurn):
    ...
    if iGameTurn == 13:
        if iPlayer == iRome and gc.getPlayer(iPlayer).isHuman():
            ...
            for i in LNewUnits:
                pUnit = gc.getPlayer(gc.getBARBARIAN_PLAYER()).initUnit(i, Landungsplot.getX(),
    ...
    if iGameTurn == 19:
        if iPlayer == iRome and gc.getPlayer(iPlayer).isHuman():
            ...
            for i in LNewUnits:
                for _ in range(4):
                    pUnit = gc.getPlayer(iCarthago).initUnit(i, Schiffsplot.getX(), Schiffsplot.getY(),
    ...
    if iGameTurn == 23:
        if iPlayer == iCarthago and gc.getPlayer(iPlayer).isHuman():
            ...
            for i in LNewUnits:
                for _ in range(2):
                    pUnit = gc.getPlayer(iRome).initUnit(i, Schiffsplot.getX(), Schiffsplot.getY(),
                                                         UnitAITypes.UNITAI_ASSAULT_SEA, DirectionTypes.DIRECTION_SOUTH)
                    pUnit.setExperience(2, -1)
                    pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_COMBAT1"), True)
            ...
            for i in LNewUnits:
                pUnit = gc.getPlayer(iRome).initUnit(i, Landungsplot.getX(), Landungsplot.getY(),
    ...
    if iGameTurn == 50:
        if iPlayer == iRome and gc.getPlayer(iPlayer).isHuman():
            ...
            if pPlot.getOwner() == iRome and gc.getTeam(gc.getPlayer(iCarthago)).isAtWar(gc.getPlayer(iRome)):
                ...
                for i in LNewUnits:
                    pUnit = gc.getPlayer(gc.getBARBARIAN_PLAYER()).initUnit(i, pPlot.getX(), pPlot.getY(),
        ...
        if iPlayer == iCarthago or iPlayer == iRome:
            ...
            if pTeamMessana.isVassal(iTeamCarthago) and gc.getTeam(gc.getPlayer(iCarthago)).isAtWar(gc.getPlayer(iRome)):
                ...
                if iPlayer == iCarthago:
                    ...
                    for i in LNewUnits:
                        for _ in range(4):
                            pUnit = gc.getPlayer(iMessana).initUnit(i, Landungsplot.getX(), Landungsplot.getY(),
                                                                    UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
                            pUnit.setExperience(2, -1)
    ...
    if iGameTurn == 125:
        if iPlayer == iCarthago:
            ...
            if pPlot.getOwner() == iCarthago and gc.getTeam(gc.getPlayer(iCarthago)).isAtWar(gc.getPlayer(iRome)):
                ...
                pUnit = gc.getPlayer(iRome).initUnit(gc.getInfoTypeForString("UNIT_LEGION_TRIBUN"), Landungsplot.getX(
                ), Landungsplot.getY(), UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
                pUnit.setExperience(2, -1)
                pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_COMBAT1"), True)
    ...
    if iGameTurn == 150:
        if iPlayer == iRome:
            ...
            if pPlot.getOwner() == iRome and gc.getTeam(gc.getPlayer(iRome)).isAtWar(gc.getPlayer(iCarthago)):
                ...
                for i in LNewUnits:
                    pUnit = gc.getPlayer(iCarthago).initUnit(i, Landungsplot.getX(), Landungsplot.getY(),
                                                             UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
                    pUnit.setExperience(2, -1)
                    pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_COMBAT1"), True)
                    pUnit.setHasPromotion(gc.getInfoTypeForString("PROMOTION_COMBAT2"), True)
