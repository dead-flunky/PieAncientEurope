    ...
    def showVotingScreen(self):
        ...
        for (i, iVoteBuildingClass, iUNTeam, bUnknown) in aiVoteBuildingClass: # K-Mod
            if gc.getGame().isDiploVote(i): # K-Mod (previously no condition)
                ...
                if gc.getGame().canHaveSecretaryGeneral(i) and gc.getGame().getSecretaryGeneral(i) != -1 and (gc.getGame().isDebugMode() or gc.getTeam(activePlayer.getTeam()).isHasMet(gc.getGame().getSecretaryGeneral(i))):# K-Mod
                    iRow = screen.appendTableRow(szTable)
...
# Mod BUG Additions Start
    ...
    def getVoteTotalColor(self, iVoteReq, iVoteTotal, iVoteCand, bWinner, bVictoryVote):
        CvUtil.pyPrint("%i %i %i" % (iVoteReq, iVoteTotal, iVoteCand))
        if not bWinner:
            return -1
...
# Mod BUG Additions End
                ...
                # PAE: changed for better UI, sowie korrigiert und neu angepasst
                iBestBuildingTeam = -1
                bestBuilding = 0
                lTeams = []
                for iLoopTeam in xrange(gc.getMAX_CIV_TEAMS()):
                ...
                for i in xrange(gc.getNumBuildingClassInfos()):
                    if (gc.getBuildingClassInfo(i).getVictoryThreshold(iLoopVC) > 0):
                        ...
                        if gc.getBuildingClassInfo(i).getMaxGlobalInstances() == 1:
                            screen.setTableText(szTable, 0, iRow, gc.getBuildingClassInfo(i).getDescription(), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                        else:
                        ...
                        iAnz = activePlayer.getTeam().getBuildingClassCount(i)
                        if iAnz > 0:
                            screen.setTableText(szTable, 2, iRow, activePlayer.getTeam().getName() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                            screen.setTableText(szTable, 3, iRow, unicode(iAnz), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                ...
                for i in xrange(gc.getNumProjectInfos()):
                    if gc.getProjectInfo(i).getVictoryThreshold(iLoopVC) > 0:
                        ...
                        screen.setTableText(szTable, 2, iRow, activePlayer.getTeam().getName() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                        screen.setTableText(szTable, 3, iRow, unicode(activePlayer.getTeam().getProjectCount(i)), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                        """
                        #check if spaceship
                ...
                if victory.getCityCulture() != CultureLevelTypes.NO_CULTURELEVEL and victory.getNumCultureCities() > 0:
                    ...
                    iRow = screen.appendTableRow(szTable)
                    # PAE better UI: show city culture target
                    szTextCityCulture = localText.getText("TXT_KEY_VICTORY_SCREEN_CITY_CULTURE", (victory.getNumCultureCities(), gc.getCultureLevelInfo(victory.getCityCulture()).getTextKey()))
...
# Mod BUG Additions End
        ...
        if sScenarioName == "FirstPunicWar":
            if gc.getPlayer(self.iActivePlayer).getCivilizationType() == gc.getInfoTypeForString("CIVILIZATION_ROME"):
                ...
                cityName = localText.getText("TXT_KEY_CITY_NAME_ROME", ("", ))

            iNumRows = screen.getTableNumRows(szTable)
            szVictoryType = u"<font=4b>" + localText.getText("TXT_KEY_VICTORY_SCREEN_FIRSTPUNICWAR_CAPITAL", ("", )) + u"</font>"
            iVictoryTitleRow = iNumRows - 1
            screen.setTableText(szTable, 0, iVictoryTitleRow, szVictoryType, "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)

            iRow = screen.appendTableRow(szTable)
            screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_FIRSTPUNICWAR_CAPITAL_2", ("", )), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
            screen.setTableText(szTable, 2, iRow, cityName, "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
            screen.appendTableRow(szTable)
            screen.appendTableRow(szTable)
        # ---------------------------------
