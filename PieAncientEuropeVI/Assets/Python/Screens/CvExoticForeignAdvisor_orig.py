...
class CvExoticForeignAdvisor(CvForeignAdvisor.CvForeignAdvisor):
	...
	def interfaceScreen(self, iScreen):
		...
		self.ATTITUDE_DICT = {
			"COLOR_GREEN": re.sub(
				":", "|", localText.getText("TXT_KEY_ATTITUDE_FRIENDLY", ())
			),
			"COLOR_CYAN": re.sub(
				":", "|", localText.getText("TXT_KEY_ATTITUDE_PLEASED", ())
			),
			"COLOR_MAGENTA": re.sub(
				":", "|", localText.getText("TXT_KEY_ATTITUDE_ANNOYED", ())
			),
			"COLOR_RED": re.sub(
				":", "|", localText.getText("TXT_KEY_ATTITUDE_FURIOUS", ())
			),
		}
		...
		for iLoopPlayer in range(gc.getMAX_PLAYERS()):
			...
			if (
				...
				screen.attachTextGFC(
					infoPanelName,
					"",
					localText.getText("TXT_KEY_CIVICS_SCREEN_TITLE", ()) + ":",
					FontTypes.GAME_FONT,
					WidgetTypes.WIDGET_GENERAL,
					-1,
					-1,
				...
				if (nFavoriteCivic != -1) and (
					not gc.getGame().isOption(
	...
	def calculateRelations(self, nPlayer, nTarget):
		if nPlayer != nTarget and gc.getTeam(gc.getPlayer(nPlayer).getTeam()).isHasMet(
			gc.getPlayer(nTarget).getTeam()
		):
			nAttitude = 0
	...
	def drawGlanceRows(self, screen, mainPanelName, bSorted=False, nPlayer=1):
		...
		for i in range(self.nCount):
			...
			if (
				iLoopPlayer != self.iActiveLeader
				and self.bReduceOnSelectedLeaders
				and not iLoopPlayer in self.listSelectedLeaders
			):
				continue
			...
			for j in range(gc.getMAX_PLAYERS()):
				if self.ltPlayerMet[j]:
					if j != self.iActiveLeader:  # TODO: Wrong selection, too.
						...
						if (
							j != self.iActiveLeader
							and self.bReduceOnSelectedLeaders
							and not j in self.listSelectedLeaders
						):
							continue
						szName = self.getNextWidgetName()
	...
	def initTradeTable(self):
		...
		gridY = (
			self.MIN_TOP_BOTTOM_SPACE
			+ self.RES_SURPLUS_HEIGHT
			+ self.RES_PANEL_SPACE
			+ self.TITLE_HEIGHT
			+ 10
		)
		...
		self.resIconGrid.setHeader(
			self.leaderCol, localText.getText("TXT_KEY_FOREIGN_ADVISOR_LEADER", ())
		)
		self.resIconGrid.setHeader(
			self.surplusCol,
		...
		if self.RES_SHOW_IMPORT_EXPORT_HEADER:
			...
			self.resIconGrid.createColumnGroup(
				localText.getText("TXT_KEY_FOREIGN_ADVISOR_EXPORT", ()), 2
			)
			self.resIconGrid.createColumnGroup(
				localText.getText("TXT_KEY_FOREIGN_ADVISOR_IMPORT", ()), 3
			)
			if self.RES_SHOW_ACTIVE_TRADE:
				self.resIconGrid.createColumnGroup(
					localText.getText("TXT_KEY_FOREIGN_ADVISOR_ACTIVE", ()), 3
				)
	...
	def calculateSurplusPanelLayout(self):
		...
		if self.RES_SHOW_SURPLUS_AMOUNT_ON_TOP:
			...
			SURPLUS_VERTICAL_SPACING = (
				self.RES_SURPLUS_HEIGHT - self.RESOURCE_ICON_SIZE - self.TITLE_HEIGHT
			) / 2
			self.SURPLUS_ICONS_Y = (
				self.SURPLUS_Y + SURPLUS_VERTICAL_SPACING + self.TITLE_HEIGHT
			)
			self.SURPLUS_TABLE_Y = (
				self.SURPLUS_ICONS_Y
				+ (self.RESOURCE_ICON_SIZE - self.TABLE_CONTROL_HEIGHT) / 2
				+ 8
			)
		else:
	...
	def drawResourceDeals(self, bInitial):
		...
		for iLoopBonus in range(gc.getNumBonusInfos()):
			...
			for iLoopPlayer in range(gc.getMAX_PLAYERS()):
				...
				if (
					...
					and not currentPlayer.isBarbarian()
					and not currentPlayer.isMinorCiv()
					and gc.getTeam(currentPlayer.getTeam()).isHasMet(
						activePlayer.getTeam()
					)
					and iLoopPlayer != self.iActiveLeader
		...
		screen.addPanel(
			self.mainAvailablePanel,
			localText.getText("TXT_KEY_FOREIGN_ADVISOR_SURPLUS_RESOURCES", ()),
			"",
			False,
			False,
			self.SURPLUS_X,
		...
		screen.addMultiListControlGFC(
			...
			"",
			self.SURPLUS_ICONS_X,
			self.SURPLUS_ICONS_Y,
			self.RESOURCE_ICON_SIZE * len(listSurplus),
			self.RESOURCE_ICON_SIZE,
			1,
			32,
			32,
		...
		if self.RES_SHOW_SURPLUS_AMOUNT_ON_TOP:
			for iIndex in range(len(listSurplus)):
				screen.addDDSGFC(
					...
					self.SURPLUS_CIRCLE_X_START + iIndex * self.RESOURCE_ICON_SIZE,
					self.SURPLUS_CIRCLE_Y,
					16,
					16,
					WidgetTypes.WIDGET_GENERAL,
					-1,
		...
		screen.addTableControlGFC(
			self.availableTable,
			len(listSurplus),
			self.SURPLUS_TABLE_X,
			self.SURPLUS_TABLE_Y,
			len(listSurplus) * self.RESOURCE_ICON_SIZE,
			self.TABLE_CONTROL_HEIGHT,
			False,
			False,
			16,
		...
		for iIndex in range(len(listSurplus)):
			...
			screen.appendMultiListButton(
				self.availableMultiList,
				gc.getBonusInfo(listSurplus[iIndex]).getButton(),
				0,
			...
			if self.RES_SHOW_SURPLUS_AMOUNT_ON_TOP:
				amountStr = (
					...
					+ localText.changeTextColor(
						str(amount), gc.getInfoTypeForString("COLOR_YELLOW")
					)
					+ "</font>"
				)
			else:
		...
		self.TABLE_PANEL_Y = (
			self.SURPLUS_Y + self.RES_SURPLUS_HEIGHT + self.RES_PANEL_SPACE
		)
		self.TABLE_PANEL_WIDTH = self.W_SCREEN - 2 * self.RES_LEFT_RIGHT_SPACE
		self.TABLE_PANEL_HEIGHT = (
			self.H_SCREEN - self.TABLE_PANEL_Y - self.RES_TOP_BOTTOM_SPACE
		)

		self.tradePanel = self.getNextWidgetName()
		screen.addPanel(
			self.tradePanel,
			localText.getText("TXT_KEY_FOREIGN_ADVISOR_TRADE_TABLE", ()),
			"",
			True,
			True,
			self.TABLE_PANEL_X,
		...
		for iLoopPlayer in range(gc.getMAX_PLAYERS()):
			...
			if (
				...
				# gold
				if (gc.getTeam(activePlayer.getTeam()).isGoldTrading()
					or gc.getTeam(currentPlayer.getTeam()).isGoldTrading()):
				...
				for iLoopBonus in range(gc.getNumBonusInfos()):
					...
					if currentPlayer.canTradeItem(self.iActiveLeader, tradeData, False):
						if currentPlayer.canTradeItem(
							self.iActiveLeader, tradeData, not currentPlayer.isHuman()
						):  # will trade
							self.resIconGrid.addIcon(
								currentRow,
								self.willTradeCol,
	...
	def drawTechDeals(self, bInitial):
		...
		for iLoopPlayer in range(gc.getMAX_PLAYERS()):
			...
			if (
				...
				message = ""
				if (
					not gc.getTeam(activePlayer.getTeam()).isTechTrading()
					and not gc.getTeam(currentPlayer.getTeam()).isTechTrading()
				):
					message = localText.getText(
				...
				if (
					gc.getTeam(activePlayer.getTeam()).isTechTrading()
					...
					for iLoopTech in range(gc.getNumTechInfos()):
						...
						if (activePlayer.canTradeItem(iLoopPlayer, tradeData, False)
							and activePlayer.getTradeDenial(iLoopPlayer, tradeData)
							== DenialTypes.NO_DENIAL):  # wants
							self.techIconGrid.addIcon(
								currentRow,
								1,
						...
						if currentPlayer.canTradeItem(
							...
							if (
								currentPlayer.getTradeDenial(
									self.iActiveLeader, tradeData
								)
	...
	def initTechTable(self):
		...
		self.techIconGrid.setHeader(
			0,
			localText.getText("TXT_KEY_FOREIGN_ADVISOR_LEADER", ())
		)
		self.techIconGrid.setHeader(
			1,
			localText.getText("TXT_KEY_FOREIGN_ADVISOR_WANTS", ())
		)
		self.techIconGrid.setHeader(
			2,
			localText.getText("TXT_KEY_FOREIGN_ADVISOR_CAN_RESEARCH", ())
		)
		self.techIconGrid.setHeader(
			3,
			(u"%c" % gc.getCommerceInfo(CommerceTypes.COMMERCE_GOLD).getChar())
		)
		self.techIconGrid.setTextColWidth(
			3,
			self.TECH_GOLD_COL_WIDTH)
		self.techIconGrid.setHeader(
			4,
			localText.getText("TXT_KEY_FOREIGN_ADVISOR_FOR_TRADE_2", ())
		)
		...
		self.techIconGrid.setHeader(
			6,
			localText.getText("TXT_KEY_FOREIGN_ADVISOR_CANT_TRADE", ())
		)
			...
			if inputClass.getData1() == 301312:
				if inputClass.getData2() == 1:
					...
					self.extendSelection(
						self.iSelectedLeader2, [0, 1, 2, 3, 4, 5], True
					)
					self.drawContents(False)
				elif inputClass.getData2() == 2:
					# Draw relation lines for all trading partner
					(listPlayers, nNumPLayers) = self.getActiveDealsMap(
						self.iSelectedLeader2
					)
					...
					if self.iShiftKeyDown == 0:
						...
						self.listSelectedLeaders = []
						self.extendSelection(
							self.iActiveLeader, [0, 1, 2, 3, 4, 5], True
						)
