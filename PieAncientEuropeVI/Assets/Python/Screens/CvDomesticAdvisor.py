...
class CvDomesticAdvisor:
  ...
  def drawHeaders( self ):
    ...
    # Cultivation status
    screen.setTableColumnHeader( "CityListBackground", 3, "<font=2>" + (u"%c" % gc.getBonusInfo(gc.getInfoTypeForString("BONUS_COW")).getChar()) + "</font>", (35 * self.nTableWidth) / self.nNormalizedTableWidth )
    
    # Settled Slaves Column
    screen.setTableColumnHeader( "CityListBackground", 4, "<font=2>" + (u"%c" % gc.getBonusInfo(gc.getInfoTypeForString("BONUS_SLAVES")).getChar()) + "</font>", (35 * self.nTableWidth) / self.nNormalizedTableWidth )

    # Settled Glads Column
    screen.setTableColumnHeader( "CityListBackground", 5, "<font=2>" + u"%c" % gc.getBonusInfo(gc.getInfoTypeForString("BONUS_BRONZE")).getChar() + "</font>", (35 * self.nTableWidth) / self.nNormalizedTableWidth )
    ...
    # Production Column
    screen.setTableColumnHeader( "CityListBackground", 9, "<font=2>" + (u"%c" % gc.getYieldInfo(YieldTypes.YIELD_PRODUCTION).getChar()) + "</font>", (35 * self.nTableWidth) / self.nNormalizedTableWidth )

    # Gold Column
    screen.setTableColumnHeader( "CityListBackground", 10, "<font=2>" + (u"%c" % gc.getCommerceInfo(CommerceTypes.COMMERCE_GOLD).getChar()) + "</font>", (35 * self.nTableWidth) / self.nNormalizedTableWidth )

    # Research Column
    szText = u"%c" %(gc.getCommerceInfo(CommerceTypes.COMMERCE_RESEARCH).getChar())
    screen.setTableColumnHeader( "CityListBackground", 11, "<font=2>" + szText, (40 * self.nTableWidth) / self.nNormalizedTableWidth )
    ...
    # Culture Column
    screen.setTableColumnHeader( "CityListBackground", 13, "<font=2>" + (u"%c" % gc.getCommerceInfo(CommerceTypes.COMMERCE_CULTURE).getChar()) + "</font>", (60 * self.nTableWidth) / self.nNormalizedTableWidth )

    # Trade Column
    screen.setTableColumnHeader( "CityListBackground", 14, "<font=2>" + (u"%c" % CyGame().getSymbolID(FontSymbols.TRADE_CHAR)) + "</font>", (35 * self.nTableWidth) / self.nNormalizedTableWidth )

    # Maintenance Column
    screen.setTableColumnHeader( "CityListBackground", 15, "<font=2>" + (u"%c" % CyGame().getSymbolID(FontSymbols.BAD_GOLD_CHAR)) + "</font>", (35 * self.nTableWidth) / self.nNormalizedTableWidth )

    # Great Person Column
    screen.setTableColumnHeader( "CityListBackground", 16, "<font=2>" + (u"%c" % CyGame().getSymbolID(FontSymbols.GREAT_PEOPLE_CHAR)) + "</font>", (70 * self.nTableWidth) / self.nNormalizedTableWidth )
    ...
    # Production Column
    screen.setTableColumnHeader( "CityListBackground", 18, "<font=2>" + localText.getText("TXT_KEY_DOMESTIC_ADVISOR_PRODUCING", ()) + "</font>", (105 * self.nTableWidth) / self.nNormalizedTableWidth )
  ...
  def drawContents (self):
    ...
    # Build the table
    screen.addTableControlGFC( "CityListBackground", 21, 18, 21, self.nTableWidth, self.nTableHeight, True, False, 24, 24, TableStyles.TABLE_STYLE_STANDARD )
  ...
  def updateTable(self, pLoopCity, i):
    ...
    if pLoopCity.getNumRealBuilding(iBuildingMetropole): buttonCityStatus = gc.getBuildingInfo(iBuildingMetropole).getButton()
    elif pLoopCity.getNumRealBuilding(iBuildingProvinz): buttonCityStatus = gc.getBuildingInfo(iBuildingProvinz).getButton()
    elif pLoopCity.getNumRealBuilding(iBuildingCity): buttonCityStatus = gc.getBuildingInfo(iBuildingCity).getButton()
    elif pLoopCity.getNumRealBuilding(iBuildingKolonie): buttonCityStatus = gc.getBuildingInfo(iBuildingKolonie).getButton()
    else: buttonCityStatus = gc.getBuildingInfo(iBuildingSiedlung).getButton() 
    ...
    # Cultivation status
    text = u"%d/%d" % (PAE_Cultivation.getCityCultivatedBonuses(pLoopCity,0), PAE_Cultivation.getCityCultivationAmount(pLoopCity,0))
    ...
    iCitySlaves2 = pLoopCity.getFreeSpecialistCount(17) # SPECIALIST_SLAVE_FOOD
    iCitySlaves3 = pLoopCity.getFreeSpecialistCount(18) # SPECIALIST_SLAVE_PROD                
    ...
    # Settled Gladiators
    iCityGlads = pLoopCity.getFreeSpecialistCount(15)  # SPECIALIST_GLADIATOR
    ...
    # Gold status...
    screen.setTableInt( "CityListBackground", 10, i, unicode(pLoopCity.getCommerceRate(CommerceTypes.COMMERCE_GOLD)), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )

    # Science rate...
    screen.setTableInt( "CityListBackground", 11, i, unicode(pLoopCity.getCommerceRate(CommerceTypes.COMMERCE_RESEARCH)), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )

    # Espionage rate...
    screen.setTableInt( "CityListBackground", 12, i, unicode(pLoopCity.getCommerceRate(CommerceTypes.COMMERCE_ESPIONAGE)), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
    ...
    screen.setTableInt( "CityListBackground", 13, i, szCulture, "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )

    # Trade
    screen.setTableInt( "CityListBackground", 14, i, unicode(pLoopCity.getTradeYield(YieldTypes.YIELD_COMMERCE)), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )

    # Maintenance...
    screen.setTableInt( "CityListBackground", 15, i, unicode(pLoopCity.getMaintenance()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
