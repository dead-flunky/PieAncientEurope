...
from CvPythonExtensions import (CyGlobalContext, CyArtFileMgr, CyTranslator,
                                WidgetTypes, FontTypes, PanelStyles, CyInterface)
...
class BugScreen:
    ...
    def addTitle(self, sWidgetId, sTxt_Key, sFont, bUpper, iX, iY, iZ):
        self.Title = BugTab(sWidgetId, sTxt_Key, sFont, bUpper, iX, iY, iZ, True,
                            True, False, None, None, WidgetTypes.WIDGET_GENERAL)

    def addTab(self, sWidgetId, sTxt_Key, sFont, bUpper, iX, iY, iZ, bShow, bEnabled, bActive, sDraw, sRefresh, WidgetType):
        self.tabs.append(BugTab(sWidgetId, sTxt_Key, sFont, bUpper, iX, iY, iZ,
                         bShow, bEnabled, bActive, sDraw, sRefresh, WidgetType))
...
#		BugUtil.debug("CvEspionage Advisor: evenlySpaceTabs %i %i", gapwidth, num_gaps)
        ...
        for itab in xrange(len(self.tabs)):
...
#				BugUtil.debug("CvEspionage Advisor: evenlySpaceTabs %i", iStart)
    ...
    def draw(self):
        ...
        self.screen.addDDSGFC(self.BGwidget_Id, ArtFileMgr.getInterfaceArtInfo(self.BGkey).getPath(),
                              0, 0, self.width, self.height, WidgetTypes.WIDGET_GENERAL, -1, -1)
        self.screen.addPanel(self.widget_id + "TopPanel", u"", u"", True, False,
                             0, 0, self.width, 55, PanelStyles.PANEL_STYLE_TOPBAR)
        self.screen.addPanel(self.widget_id + "BottomPanel", u"", u"", True, False, 0,
                             713, self.width, 55, PanelStyles.PANEL_STYLE_BOTTOMBAR)
