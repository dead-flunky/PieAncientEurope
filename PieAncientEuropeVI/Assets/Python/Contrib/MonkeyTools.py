...
from CvPythonExtensions import (CyGlobalContext, CyTranslator, CyInterface,
                                CyGame, CyMap, ActivityTypes, MissionAITypes,
                                MissionTypes, DomainTypes, TeamTypes)
import CvUtil
import PyHelpers
    ...
    def doReInit(self):
        ...
        for ii in xrange(len(unitList)):
            ...
            if (pGroup.getActivityType() == ActivityTypes.ACTIVITY_INTERCEPT):
                ...
                pGroup.pushMission(MissionTypes.MISSION_FORTIFY, 0, 0, 0, False, False,
                                   MissionAITypes.NO_MISSIONAI, pPlot, pLoopUnit)
                # add new mission -> air patrol
                pGroup.pushMission(MissionTypes.MISSION_AIRPATROL, 0, 0, 0, False,
                                   False, MissionAITypes.NO_MISSIONAI, pPlot, pLoopUnit)
...
def getPromotionInfoText(pUnit):
    ...
    if iEnemyHealChange > 0:
        szPromotionInfo += localText.getText("TXT_KEY_PROMOTION_HEALS_EXTRA_TEXT", (iEnemyHealChange, )) + \
            localText.getText("TXT_KEY_PROMOTION_ENEMY_LANDS_TEXT", ()) + "\n"
    if iNeutralHealChange > 0:
        szPromotionInfo += localText.getText("TXT_KEY_PROMOTION_HEALS_EXTRA_TEXT", (iNeutralHealChange, )) + \
            localText.getText("TXT_KEY_PROMOTION_NEUTRAL_LANDS_TEXT", ()) + "\n"
    if iFriendlyHealChange > 0:
        szPromotionInfo += localText.getText("TXT_KEY_PROMOTION_HEALS_EXTRA_TEXT", (iFriendlyHealChange, )) + \
            localText.getText("TXT_KEY_PROMOTION_FRIENDLY_LANDS_TEXT", ()) + "\n"
    if iSameTileHealChange > 0:
        szPromotionInfo += localText.getText("TXT_KEY_PROMOTION_HEALS_SAME_TEXT", (iSameTileHealChange, )) + \
            localText.getText("TXT_KEY_PROMOTION_DAMAGE_TURN_TEXT", ()) + "\n"
    if iAdjacentTileHealChange > 0:
        szPromotionInfo += localText.getText("TXT_KEY_PROMOTION_HEALS_ADJACENT_TEXT", (iAdjacentTileHealChange, )
                                             ) + localText.getText("TXT_KEY_PROMOTION_DAMAGE_TURN_TEXT", ()) + "\n"
    if iCombatPercent > 0:
