...
from CvPythonExtensions import (CyGlobalContext, YieldTypes, WidgetTypes)
import CvUtil
import BugUtil
import TradeUtil
...
class Tracker:
    ...
    def _addTile(self, eFirstTileType, pPlot):
        for eYield in YIELDS:
            ...
            for eType in xrange(eFirstTileType, ALL_TILES + 1):
    ...
    def calculateSpecialists(self, pCity):
        ...
        for eYield in xrange(YieldTypes.NUM_YIELD_TYPES):
            ...
            for eSpec in xrange(gc.getNumSpecialistInfos()):
                iValue += pPlayer.specialistYield(eSpec, eYield) * \
                    (pCity.getSpecialistCount(eSpec) + pCity.getFreeSpecialistCount(eSpec))
    ...
    def appendTable(self, screen, table, bTotal, heading, eYield, iValue, bFraction=False):
        ...
        if bTotal:
            ...
            if bFraction:
                ...
                value = u"%+d" % iValue
        screen.setTableText(table, HEADING_COLUMN, self.iRow, u"<font=1>%s</font>" %
                            (heading), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
        screen.setTableText(table, VALUE_COLUMN, self.iRow, u"<font=1>%s%c</font>" %
                            (value, cYield), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_RIGHT_JUSTIFY)
        self.iRow += 1
    ...
    def appendTableTotal(self, screen, table, eYield, iValue):
        ...
        if self.iRow > 0:
            cYield = gc.getYieldInfo(eYield).getChar()
            value = u"<color=205,180,55,255>%d</color>" % iValue
            screen.setTableText(table, TOTAL_COLUMN, self.iRow - 1, u"<font=1>%s%c</font>" %
                                (value, cYield), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_RIGHT_JUSTIFY)
