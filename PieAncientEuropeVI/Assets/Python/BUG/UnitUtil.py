...
from CvPythonExtensions import (CyGlobalContext, ActivityTypes, AutomateTypes,
                                CyGame, GameOptionTypes, DomainTypes,
                                MissionTypes)
import BugUtil
import PlayerUtil
...
def init():
    ...
    for eUnit in xrange(NUM_UNITS):
        ...
        for i in xrange(NUM_OR_BONUSES):
...
def getOlderUnits(eUnit):
    ...
    for eOldUnit in xrange(NUM_UNITS):
...
def getNewerUnits(eUnit):
    ...
    for eNewUnit in xrange(NUM_UNITS):
        ...
        if upgrades:
            for eUpgradeUnit in upgrades:
                if eUpgradeUnit not in units and eUpgradeUnit not in generics:
                    # BugUtil.debug("findObsoleteUnits - %s, cannot build %s", unitInfo.getDescription(), gc.getUnitInfo(eUpgradeUnit).getDescription())
                    break
            else:
                obsoletes.add(eUnit)
    return obsoletes
