...
def canTrade(playerOrID, withPlayerOrID):
    ...
    Returns True if <player> can open the trade window with <withPlayer>.
    """
    return PlayerUtil.canContact(playerOrID, withPlayerOrID) and PlayerUtil.isWillingToTalk(withPlayerOrID, playerOrID)
...
def test(player, type, value):
    CvUtil.pyPrint(format(player, makeTrade(type, value)))
    ...
    for eItem, name in STATUS_TRADE_ITEMS:
        ...
        if not can:
            if will:
                ...
                CvUtil.pyPrint("%s: can't and won't because %s" % (name, denial))
        else:
            if will:
