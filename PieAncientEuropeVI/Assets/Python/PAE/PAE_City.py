...
def onModNetMessage(argsList):
    ...
    if iData1 == 678:
        ...
        if bDoRebellion:
            ...
            if iAuxiliar == -1: iAuxiliar = gc.getInfoTypeForString("UNIT_AUXILIAR")
    ...
    if iData1 == 737:
        ...
        # -- Tribut fordern --
            ...
            if iUnit2 == -1: iUnit2 = gc.getInfoTypeForString("UNIT_AUXILIAR")
            iUnhappy2 = 1
...
def doCheckCityState(pCity):
    ...
    if pCity.getPopulation() >= iPopDorf and pCity.getNumRealBuilding(iBuildingKolonie) == 0:
        ...
        if pCity.getProductionProcess() != -1: pCity.clearOrderQueue()
...
def doCityCheckRevoltEnd(pCity):
    ...
    for iUnit in range(iRange):
      ...
      if pUnit.isHasPromotion(gc.getInfoTypeForString("PROMOTION_RHETORIK")) or pUnit.isHasPromotion(gc.getInfoTypeForString("PROMOTION_LEADER")):
        ...
        pPlayer = gc.getPlayer(pCity.getOwner())
        if pPlayer.isHuman():
          CyInterface().addMessage(pCity.getOwner(), True, 10, CyTranslator().getText("TXT_KEY_INFO_CIVIL_WAR_1", (pCity.getName(),)), "AS2D_WELOVEKING", 2, gc.getBuildingInfo(gc.getInfoTypeForString("BUILDING_CIVIL_WAR")).getButton(), ColorTypes(8), pCity.getX(), pCity.getY(), True, True)
        return True
    return False
...
def doEmigrant(pCity, pUnit):
    ...
    if iPlayerHC == -1: iPlayerHC = pCity.getOwner()
...
def doRenegadeOnCombatResult(pLoser, pCity, iWinnerPlayer):
    ...
    if pCity.hasTrait(gc.getInfoTypeForString("TRAIT_PROTECTIVE")): return False
    ...
    if pCity.getOwner() == gc.getBARBARIAN_PLAYER(): return False
    ...
    if pCity.isHasBuilding(iBuilding):
        ...
        if bCityRenegade:
            ...
            # Message
            if pWinnerPlayer.isHuman():
                if iWinnerPlayer == gc.getGame().getActivePlayer(): CyAudioGame().Play2DSound('AS2D_REVOLTSTART')
                popupInfo = CyPopupInfo()
                popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON)
            ...
            if pLoserPlayer.isHuman():
                if iLoserPlayer == gc.getGame().getActivePlayer(): CyAudioGame().Play2DSound('AS2D_REVOLTSTART')
                popupInfo = CyPopupInfo()
                popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_TEXT)
        ...
        if PlotArray:
            ...
            if iMaintainUnits > 0 and iCityUnits * 3 <= iEnemyUnits:
                ...
                if iReservists > 0:
                    ...
                    if iUnit == -1: iUnit = gc.getInfoTypeForString("UNIT_AUXILIAR")
                    ...
                    if iUnit == -1: iUnit = gc.getInfoTypeForString("UNIT_REFLEX_ARCHER")
                    if pCity.canTrain(iUnit, 0, 0):
                        lResUnits.append(iUnit)
                    else:
        ...
        if pPlayer.isHuman():
          ...
          if iPlayer == gc.getGame().getActivePlayer():
            ...
            if bDoRebellion:
                ...
                if iUnit != -1: lGift.append(iUnit)
                else: lGift.append(gc.getInfoTypeForString("UNIT_AUXILIAR"))
...
def removeNoBonusNoBuilding(pCity):
    ...
    lBuildings = [
        ...
        #gc.getInfoTypeForString("BUILDING_HUNDEZUCHT"),
    ...
    for building in lBuildings:
        if pCity.isHasBuilding(building):
            if CvUtil.myRandom(5, "removeNoBonusNoBuilding4") == 1:
...
def doPlagueEffects(pCity):
                ...
                if pLoopUnit is not None:
                    ...
                    if psOwner is not None and psOwner.isHuman():
                        if sOwner not in lMessageOwners:
                            ...
                            CyInterface().addMessage(sOwner, True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_CITY_PEST_2", (pCity.getName(), 0)), None, 2, 'Art/Interface/Buttons/Actions/button_skull.dds', ColorTypes(12), iX, iY, True, True)
    ...
    if iHappiness >= -1:
        ...
        # Message
        if pPlayer.isHuman():
            CyInterface().addMessage(iPlayer, True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_CITY_PEST_DONE", (pCity.getName(), iNewPop, iOldPop)), "AS2D_WELOVEKING", 2, 'Art/Interface/Buttons/Actions/button_skull.dds', ColorTypes(8), pCity.getX(),  pCity.getY(), True, True)
...
def catchGreatPeople(pCity, iNewOwner, iPreviousOwner, bAssimilation):
    ...
    if bAssimilation:
        ...
        lNumGP = [
            pCity.getFreeSpecialistCount(8),  # SPECIALIST_GREAT_PRIEST
            pCity.getFreeSpecialistCount(9),  # SPECIALIST_GREAT_ARTIST
            pCity.getFreeSpecialistCount(10),  # SPECIALIST_GREAT_SCIENTIST
            pCity.getFreeSpecialistCount(11),  # SPECIALIST_GREAT_MERCHANT
            pCity.getFreeSpecialistCount(12),  # SPECIALIST_GREAT_ENGINEER
            pCity.getFreeSpecialistCount(13),  # SPECIALIST_GREAT_GENERAL
            pCity.getFreeSpecialistCount(14)  # SPECIALIST_GREAT_SPY
        ]
                    ...
                    if iLoopOwner != -1 and iLoopOwner != iNewOwner and iLoopOwner != gc.getBARBARIAN_PLAYER():
                        ...
                        if iRand == 1:
                            ...
                            if gc.getPlayer(iLoopOwner).isHuman():
                                CyInterface().addMessage(iLoopOwner, True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_CITY_GETS_POP", (pCity.getName(), loopCity.getName())), None, 2, "Art/Interface/Buttons/Units/button_emigrant.dds", ColorTypes(13), loopPlot.getX(), loopPlot.getY(), True, True)
                            # Kultur
                    ...
                    for iLoopPlayer in range(gc.getMAX_PLAYERS()):
                        ...
                        if pLoopTeam.isHasMet(pPlayer.getTeam()) and pLoopPlayer.isHuman():
                            if iLoopPlayer == iOwner:
                                CyInterface().addMessage(iLoopPlayer, False, 10, CyTranslator().getText("TXT_KEY_WONDER_CAPTURE_YOU", (pPlayer.getName(), eLoopBuilding.getDescription())), '', 0, eLoopBuilding.getButton(), ColorTypes(8), pCity.getX(), pCity.getY(), True, True)
                            else:
                                CyInterface().addMessage(iLoopPlayer, False, 10, CyTranslator().getText("TXT_KEY_WONDER_CAPTURE", (pPlayer.getName(), eLoopBuilding.getDescription())), '', 0, eLoopBuilding.getButton(), ColorTypes(7), pCity.getX(), pCity.getY(), True, True)
...
def doCheckCityName(pCity):
  ...
  if sScenarioName == "RiseOfGreece": Filename = "Cities_RiseOfGreece.txt"
  ...
  if Filename != "":
    ...
    for CurString in MyFile.readlines():
      ...
      if "x=" in CurString or "X=" in CurString:
        ...
        if "TXT_KEY" in cityName: cityName = CyTranslator().getText(cityName,())
          ...
          for iPlayer in range (iNumPlayers):
            ...
            for iCity in range (iNumCities):
              if pPlayer.getCity(iCity).getName() == cityName:
                bRename = False
                break # for iCity
            if not bRename: break # for iPlayer
...
def getCityStatus(pCity, iPlayer, iCity, bReturnButton):
    if pCity == None: pCity = gc.getPlayer(iPlayer).getCity(iCity)
    if pCity.isHasBuilding(gc.getInfoTypeForString("BUILDING_METROPOLE")): 
      if bReturnButton: return gc.getBuildingInfo(gc.getInfoTypeForString("BUILDING_METROPOLE")).getButton()
      else: return 4
    elif pCity.isHasBuilding(gc.getInfoTypeForString("BUILDING_PROVINZ")):
      if bReturnButton: return gc.getBuildingInfo(gc.getInfoTypeForString("BUILDING_PROVINZ")).getButton()
      else: return 3
    elif pCity.isHasBuilding(gc.getInfoTypeForString("BUILDING_STADT")):
      if bReturnButton: return gc.getBuildingInfo(gc.getInfoTypeForString("BUILDING_STADT")).getButton()
      else: return 2
    elif pCity.isHasBuilding(gc.getInfoTypeForString("BUILDING_KOLONIE")):
      if bReturnButton: return gc.getBuildingInfo(gc.getInfoTypeForString("BUILDING_KOLONIE")).getButton()
      else: return 1
    
    if bReturnButton: return gc.getBuildingInfo(gc.getInfoTypeForString("BUILDING_SIEDLUNG")).getButton()
    else: return 0
...
def doCheckCivilWar(pCity):
  if pCity.isHasBuilding(gc.getInfoTypeForString("BUILDING_CIVIL_WAR")):
    ...
    if iPop <= iMilitaryUnits + iUnitsExtraCalc or iChance < 5 or not bHuman and iChance < 10:
      ...
      if pPlayer.isHuman():
        CyInterface().addMessage(iPlayer, True, 10, CyTranslator().getText("TXT_KEY_INFO_CIVIL_WAR_1", (pCity.getName(),)), "AS2D_WELOVEKING", 2, gc.getBuildingInfo(gc.getInfoTypeForString("BUILDING_CIVIL_WAR")).getButton(), ColorTypes(8), pCity.getX(), pCity.getY(), True, True)
        #popupInfo = CyPopupInfo()
        #popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_TEXT)
      ...
      if pPlayer.isHuman():
        CyInterface().addMessage(iPlayer, True, 10, CyTranslator().getText("TXT_KEY_INFO_CIVIL_WAR_6", (pCity.getName(), iMilitaryUnits, iPop)), "AS2D_REVOLTSTART", 2, gc.getBuildingInfo(gc.getInfoTypeForString("BUILDING_CIVIL_WAR")).getButton(), ColorTypes(7), pCity.getX(), pCity.getY(), True, True)
      ...
      if iChance < 25 or bHuman and iChance < 50:
        ...
        # es wird eine Einheit gekillt
          ...
          if len(lHarmedUnits):
            ...
            if pPlayer.isHuman():
              CyInterface().addMessage(iPlayer, True, 10, CyTranslator().getText("TXT_KEY_INFO_CIVIL_WAR_3", (pCity.getName(),)), None, 2, gc.getBuildingInfo(gc.getInfoTypeForString("BUILDING_CIVIL_WAR")).getButton(), ColorTypes(7), pCity.getX(), pCity.getY(), True, True)
              popupInfo = CyPopupInfo()
              popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_TEXT)
          ...
          if iMilitaryUnits <= 0:
            ...
            if pPlayer.isHuman():
              CyInterface().addMessage(iPlayer, True, 10, CyTranslator().getText("TXT_KEY_INFO_CIVIL_WAR_4", (pCity.getName(),)), "AS2D_REVOLTSTART", 2, gc.getBuildingInfo(gc.getInfoTypeForString("BUILDING_CIVIL_WAR")).getButton(), ColorTypes(7), pCity.getX(), pCity.getY(), True, True)
              popupInfo = CyPopupInfo()
              popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_TEXT)
...
def doStartCivilWar(pCity, iChance):
    ...
    if CvUtil.myRandom(100, "iRandStartCityCivilWar") < iChance:
      ...
      if pPlayer.isHuman():
        CyInterface().addMessage(iPlayer, True, 10, CyTranslator().getText("TXT_KEY_INFO_CIVIL_WAR_5", (pCity.getName(),)), "AS2D_REVOLTSTART", 2, gc.getBuildingInfo(gc.getInfoTypeForString("BUILDING_CIVIL_WAR")).getButton(), ColorTypes(7), pCity.getX(), pCity.getY(), True, True)
        popupInfo = CyPopupInfo()
        popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_TEXT)
