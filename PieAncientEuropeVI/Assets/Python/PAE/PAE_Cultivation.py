...
def getCityCultivationAmount(pCity, iTyp):
    if iTyp == 1: return 4 # 1: wenn sich alles konkurrieren soll
...
def getCityCultivatedBonuses(pCity, iTyp):
    ...
    for i in range(gc.getNUM_CITY_PLOTS()):
        ...
        if pLoopPlot is not None and not pLoopPlot.isNone():
            ...
            if iLoopBonus in L.LBonusPlantation: continue
...
def isBonusCultivationChance(iPlayer, pPlot, eBonus, bVisibleOnly=True, pCity=None):
    ...
    """
    # iTyp = Grain/Livestock or Strategics (Horse, Camel, Elefant, Dog)
    iTyp = getBonusCultivationType(eBonus)
    if iTyp == 1: List = L.LBonusStratCultivatable
    else: List = L.LBonusCultivatable
    ...
    for pCity in lCities:
        if eBonus in L.LBonusPlantation or eBonus in L.LBonusStratCultivatable:
    ...
    for iI in range(gc.getNUM_CITY_PLOTS()):
        ...
        if pLoopPlot is not None and not pLoopPlot.isNone():
            ...
            if ePlotBonus == -1 and isBonusCultivationChance(iPlayer, pLoopPlot, eBonus, False, pCity):
                ...
                elif iImprovement in lPrio3Imps: lPrio3.append(pLoopPlot)
                elif iImprovement in lPrio4Imps: lPrio4.append(pLoopPlot)
                elif gc.getImprovementInfo(iImprovement).getDefenseModifier() > 0: lPrio5.append(pLoopPlot)

    lPlotList = []
    if len(lPrio1): lPlotList = lPrio1
    elif len(lPrio2): lPlotList = lPrio2
    elif len(lPrio3): lPlotList = lPrio3
    elif len(lPrio4): lPlotList = lPrio4
    elif len(lPrio5): lPlotList = lPrio5
            ...
            if eBonusOnBoard != -1:
                ...
                if _bonusIsCultivatableFromCity(iPlayer, pLoopCity, eBonusOnBoard, False):
                    if pUnit.atPlot(pLoopCity.plot()):
                        ...
                        if pPlot != None: doCultivateBonus(pPlot, pUnit, eBonusOnBoard)
                    else:
...
def doCollectBonus4Cultivation(pUnit):
    ...
    if eBonus not in L.LBonusCultivatable + L.LBonusStratCultivatable:
      return False
    ...
    if gc.getPlayer(pUnit.getOwner()).isHuman():
        CyInterface().addMessage(pUnit.getOwner(), True, 5, CyTranslator().getText("TXT_KEY_MESSAGE_TRADE_COLLECT_GOODS", (gc.getBonusInfo(eBonus).getDescription(), -iPrice)), "AS2D_COINS", 2, None, ColorTypes(13), pUnit.getX(), pUnit.getY(), False, False)
...
def _getCollectableGoods4Cultivation(pCity):
    lGoods = []
    for eBonus in L.LBonusCultivatable + L.LBonusStratCultivatable:
...
def wine(pCity):
    ...
    for iI in range(gc.getNUM_CITY_PLOTS()):
        ...
        if loopPlot is not None and not loopPlot.isNone():
            ...
            if loopPlot.getBonusType(-1) == eBonus: return []
