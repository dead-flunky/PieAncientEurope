...
def doPopupAutomatedTradeRoute(pUnit, iType, iData1, iData2):
    ...
    if iType == 1 or iType == 4:
        ...
        # Erster Button type 4: Unsere CIV oder zu Schritt 1
            ...
            # else:
            popupInfo.addPythonButton(CyTranslator().getText("TXT_KEY_ACTION_BACK_STEP_1", ()),
                                      "Art/Interface/Buttons/Actions/Cancel.dds")
                    ...
                    while loopCity:
                        if not loopCity.isNone() and loopCity.getOwner() == iLoopPlayer:  # only valid cities
                            if loopCity.isCapital():
                                if bWater:
                                    iDistance = plotDistance(pUnitPlot.getX(), pUnitPlot.getY(),
                                                             loopCity.getX(), loopCity.getY()) - 1
                                else:
        ...
        for [pNeighborCity, lNeighborCityLuxury] in lNeighborLuxuryCities:
            for [pPlayerCity, lPlayerCityLuxury] in lPlayerLuxuryCities:
                ...
                if bWater and pNeighborCity.isCoastal(4):
                    iDistance = plotDistance(pCityPlotPlayer.getX(), pCityPlotPlayer.getY(),
                                             pCityPlotNeighbor.getX(), pCityPlotNeighbor.getY()) - 1
                else:
    ...
    if eCityBonus != -1 and eCityBonus == eBonus:
        ...
        if TechArray and len(TechArray) > 0:
            ...
            if pPlayer.isHuman():
                popupInfo = CyPopupInfo()
                popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_TEXT)
                popupInfo.setText(CyTranslator().getText("TXT_KEY_MESSAGE_POPUP_GETTING_TECH_2",
            ...
            if not pUnit.isHasPromotion(iPromo):
                ...
                if gc.getPlayer(iPlayer).isHuman():
                    ...
                    if gc.getPlayer(pPlot.getOwner()).isHuman():
                        CyInterface().addMessage(pPlot.getOwner(), True, 5, CyTranslator().getText("TXT_KEY_MESSAGE_MERCHANT_ROBBERY_1_1", (gc.getPlayer(pUnit.getOwner()
                                                                                                                                                         ).getCivilizationAdjective(3), 0)), None, 2, "Art/Interface/Buttons/Units/button_merchant.dds", ColorTypes(14), pPlot.getX(), pPlot.getY(), True, True)
