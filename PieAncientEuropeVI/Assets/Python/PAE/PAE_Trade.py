...
def doBuyBonus(pUnit, eBonus, iCityOwner):
    ...
    if eBonus != -1:
        ...
        if pSeller.isHuman() and iBuyer != iCityOwner:
            ...
            CyInterface().addMessage(iCityOwner, True, 10, CyTranslator().getText("TXT_KEY_BONUS_BOUGHT", (pBuyer.getName(), pBuyer.getCivilizationShortDescriptionKey(), pUnit.plot().getPlotCity().getName(), sBonusName, iGewinnGold)), "AS2D_COINS", 2, pUnit.getButton(), ColorTypes(8), pUnit.getX(), pUnit.getY(), True, True)
        ...
        pUnit.finishMoves()
        if pUnit.isHuman(): PAE_Unit.doGoToNextUnit(pUnit)
        ...
        if pSource is not None and not pSource.isNone():
            if pSource.isCity():
                ...
                if pPlotTradeRoad != None:
                    ...
                    if pBuyer.isHuman():
                        CyInterface().addMessage(iBuyer, True, 10, CyTranslator().getText("TXT_KEY_TRADE_ROUTE_BUILT", (pSeller.getName(), pSeller.getCivilizationShortDescriptionKey(), pCity.getName(), pCity2.getName())), "AS2D_WELOVEKING", 2, "Art/Terrain/Routes/handelsstrasse/button_handelsstrasse.dds", ColorTypes(10), pPlotTradeRoad.getX(), pPlotTradeRoad.getY(), True, True)
        ...
        if not pCity.isHasBuilding(iBuilding):
            ...
            if iAnz >= iMax:
                ...
                if pCity.getOwner() == gc.getGame().getActivePlayer():
                    CyInterface().addMessage(pCity.getOwner(), True, 10, CyTranslator().getText("TXT_KEY_TRADE_ROUTE_HANDELSZENTRUM", (pCity.getName(),)), "AS2D_WELOVEKING", 2, gc.getBuildingInfo(iBuilding).getButton(), ColorTypes(10), pCity.getX(), pCity.getY(), True, True)
...
def doPopupAutomatedTradeRoute(pUnit, iType, iData1, iData2):
    ...
    if iType == 1 or iType == 4:
        ...
        for iPlayer in lCivs:
            ...
            if iPlayer == iUnitOwner: sText = CyTranslator().getText("TXT_KEY_POPUP_TRADE_INFO_6", (gc.getPlayer(pUnit.getOwner()).getCivilizationDescription(0),))
            else: sText = pPlayer.getCivilizationDescription(0)
            if iType == 1: sText += u" (%s)" % pPlayer.getName()
            else:
                if iBonus != -1:
                    if pPlayer.hasBonus(iBonus): sText += u" %c (%s %c)" % (CyGame().getSymbolID(FontSymbols.POWER_CHAR) + 6, CyTranslator().getText("TXT_KEY_CIV_HABEN_BEREITS", ()), gc.getBonusInfo(iBonus).getChar())
                    else: sText += u" %c" % (CyGame().getSymbolID(FontSymbols.HAPPY_CHAR))
    ...
    # Choose city 1 (iType == 2) or city 2 (iType == 5)
        ...
        for pCity in lCities:
            ...
            if pCity.isCapital(): sText += CyTranslator().getText(" [ICON_STAR]", ())
            elif pCity.isHasBuilding(gc.getInfoTypeForString("BUILDING_PROVINZPALAST")): sText += CyTranslator().getText(" [ICON_SILVER_STAR]", ())
            if iType == 5:
...
def getCitySaleableGoods(pCity, iBuyer):        
    ...
    for x in range(5): # check plots
        for y in range(5):
            if x == 0 and y == 0 or x == 4 and y == 0 or x == 4 and y == 0 or x == 4 and y == 4: continue
...
def _doCheckCitySpecialBonus(pUnit, pCity, eBonus):
    ...
    if eCityBonus != -1 and eCityBonus == eBonus:
        ...
        if iUnit != -1: lUnits.append(iUnit)
        else: lUnits.append(gc.getInfoTypeForString("UNIT_AXEMAN2"))
        iUnit = eCiv.getCivilizationUnits(gc.getInfoTypeForString("UNITCLASS_REFLEX_ARCHER"))
        if iUnit != -1: lUnits.append(iUnit)
        else: lUnits.append(gc.getInfoTypeForString("UNIT_REFLEX_ARCHER"))
        iUnit = eCiv.getCivilizationUnits(gc.getInfoTypeForString("UNITCLASS_KAMPFHUND"))
        if iUnit != -1: lUnits.append(iUnit)
        else: lUnits.append(gc.getInfoTypeForString("UNIT_KAMPFHUND"))
        iUnit = eCiv.getCivilizationUnits(gc.getInfoTypeForString("UNITCLASS_SKIRMISHER"))
        if iUnit != -1: lUnits.append(iUnit)
        else: lUnits.append(gc.getInfoTypeForString("UNIT_SKIRMISHER"))
