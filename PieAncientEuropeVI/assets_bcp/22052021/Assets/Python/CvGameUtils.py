...
class CvGameUtils:
    ...
    def updateColoredPlots(self):
        ...
        if pHeadSelectedUnit is not None and not pHeadSelectedUnit.isNone():
            ...
            if not CyInterface().isCityScreenUp():
                  ...
                  for x in range(iMapW):
                      for y in range(iMapH):
                          ...
                          if loopPlot is not None and not loopPlot.isNone():
                              ...
                              if loopPlot.isWater() or loopPlot.isPeak() or loopPlot.isCity(): continue
              ...
              elif iUnitType == gc.getInfoTypeForString("UNIT_WORKBOAT"):
                  iMapW = gc.getMap().getGridWidth()
                  iMapH = gc.getMap().getGridHeight()
                  iDarkIce = gc.getInfoTypeForString("FEATURE_DARK_ICE")
                  for x in range(iMapW):
                      for y in range(iMapH):
                          ...
                          if loopPlot is not None and not loopPlot.isNone():
                              if loopPlot.getFeatureType() == iDarkIce: continue
                              if not loopPlot.isWater(): continue
                              if loopPlot.getOwner() == iPlayer:
                                  eBonus = loopPlot.getBonusType(iPlayer)
                                  if eBonus != -1:
                                      iImprovement = loopPlot.getImprovementType()
                  ...
                  for x in range(iMapW):
                      for y in range(iMapH):
                          ...
                          if loopPlot is not None and not loopPlot.isNone():
                              if loopPlot.getFeatureType() == iDarkIce: continue
                              if loopPlot.isWater() or loopPlot.isPeak() or loopPlot.isCity(): continue
                              if loopPlot.getOwner() != -1 or loopPlot.getBonusType(iPlayer) == -1: continue
              ...
              # Great General
                  ...
                  if pTeam.isHasTech(gc.getInfoTypeForString("TECH_MILIT_STRAT")):
                    ...
                    while loopCity:
                      if loopCity is not None and not loopCity.isNone():
                          if loopCity.isHasBuilding(iBuilding1):
                      ...
                      for x in range(iMapW):
                          for y in range(iMapH):
                              ...
                              if loopPlot is not None and not loopPlot.isNone():
                                  if loopPlot.getFeatureType() == iDarkIce: continue
                                  if loopPlot.isWater() or loopPlot.isPeak() or loopPlot.isCity(): continue
                                  if loopPlot.getOwner() != iPlayer or loopPlot.getBonusType(iPlayer) == -1: continue
    ...
    def isActionRecommended(self, argsList):
        ...
        elif gc.getActionInfo(iAction).getMissionType() == MissionTypes.MISSION_CONSTRUCT: return True
        return False
    ...
    def canBuild(self, argsList):
        ...
        if iBuild == gc.getInfoTypeForString("BUILD_LATIFUNDIUM"):
          ...
          if pPlot.getFeatureType() == gc.getInfoTypeForString("FEATURE_DARK_ICE"): return 0
          if pPlot.isWater() or pPlot.isPeak() or pPlot.isCity(): return 0
          if pPlot.getImprovementType() != -1: return 0
          if iBuild == gc.getInfoTypeForString("BUILD_KASTELL") and not pPlot.isFlatlands(): return 0
          return 1
    ...
    def cannotTrain(self, argsList):
        ...
        if eUnit in L.LTradeUnits:
            ...
            if iAnz1 + iAnz2 + iAnz3 >= iMax: return True
        ...
        if not pOwner.isHuman():
            ...
            if iUnitType in L.LCultivationUnits:
                ...
                if pOwner.getUnitClassCount(pUnit.getUnitClassType()) > 1:
                    ...
                    pUnit.kill(True, -1)
                else: pUnit.getGroup().pushMission(MissionTypes.MISSION_SKIP, 0, 0, 0, True, False, MissionAITypes.NO_MISSIONAI, pUnit.plot(), pUnit)
                return True
            ...
            if iUnitType == gc.getInfoTypeForString("UNIT_DRUIDE"):
                ...
                for iUnit in range(pPlot.getNumUnits()):
                  ...
                  if loopUnit.getOwner() == iOwner:
                    if loopUnit.getUnitType() == iUnitSlave: bSlaves = True
            ...
            if pPlot.isCity():
                ...
                if iUnitType == gc.getInfoTypeForString("UNIT_CARAVAN") or iUnitType == gc.getInfoTypeForString("UNIT_TRADE_MERCHANT"):
                    ...
                    if not pUnit.isHasPromotion(iPromo):
                        iCost = 20
                        if pOwner.isCivic(gc.getInfoTypeForString("CIVIC_SOELDNERTUM")): iCost = 15                    
                        if pOwner.getGold() > iCost:
                            pUnit.setHasPromotion(iPromo, True)
                            pUnit.finishMoves()
                            return True
...
## ---------------------- ##
        ...
        if eWidgetType == WidgetTypes.WIDGET_GENERAL:
            ...
            # ID 675 vergeben durch Revolten-PopUp (CvScreensInterface - popupRevoltPayment)
                ...
                if iData2 == 1: return CyTranslator().getText("TXT_KEY_MESSAGE_GOLDKARREN", ())
                else: return CyTranslator().getText("TXT_KEY_HELP_GOLDKARREN2GOVCENTER", ())
            ...
            # iData2 = 2: UNIT_SUPPLY_WAGON: Nahrung aufnehmen
                ...
                if iData2 == 1: return CyTranslator().getText("TXT_KEY_HELP_GETREIDE_ABLIEFERN", (PAE_Unit.getUnitSupplyFood(),))
                elif iData2 == 2: return CyTranslator().getText("TXT_KEY_HELP_GETREIDE_AUFNEHMEN", ())
            # Karte zeichnen
            elif iData1 == 728:
            ...
            # Cultivation / Trade
                ...
                if bOption:
                    if iData2 == -1: return CyTranslator().getText("TXT_KEY_BUTTON_COLLECT_BONUS2", ())
                    elif iData2 == 0: return CyTranslator().getText("TXT_KEY_BUTTON_COLLECT_BONUS", ())
                    else: return CyTranslator().getText("TXT_KEY_BUTTON_COLLECT_BONUS_CITY", ())
                else:
            ...
            # Allgemeine Infos (aktionslose Buttons)
                ...
                elif iData2 == 2: return CyTranslator().getText("TXT_KEY_MONUMENT_INFO_1", ())
                elif iData2 == 3: return CyTranslator().getText("TXT_KEY_MONUMENT_INFO_2", ())
                elif iData2 == 4: return CyTranslator().getText("TXT_KEY_MONUMENT_INFO_3", ())
                # Dienstgrade Info im TechChooser
                elif iData2 == 5: return CyTranslator().getText("TXT_KEY_RANK_START_INFO_GER", ())
                elif iData2 == 6: return CyTranslator().getText("TXT_KEY_RANK_START_INFO_EGYPT", ())
                elif iData2 == 7: return CyTranslator().getText("TXT_KEY_RANK_START_INFO_SUMER", ())
                elif iData2 == 8: return CyTranslator().getText("TXT_KEY_RANK_START_INFO_ASSUR", ())
                elif iData2 == 9: return CyTranslator().getText("TXT_KEY_RANK_START_INFO_SPARTA", ())
                elif iData2 == 10: return CyTranslator().getText("TXT_KEY_RANK_START_INFO_MACEDON", ())
                elif iData2 == 11: return CyTranslator().getText("TXT_KEY_RANK_START_INFO_PERSIA", ())
                elif iData2 == 12: return CyTranslator().getText("TXT_KEY_RANK_START_INFO_GREEK", ())
                elif iData2 == 13: return CyTranslator().getText("TXT_KEY_RANK_START_INFO_CARTH", ())
                elif iData2 == 14: return CyTranslator().getText("TXT_KEY_RANK_START_INFO_ROM", ())
                elif iData2 == 15: return CyTranslator().getText("TXT_KEY_RANK_START_INFO_ROM_EQUES", ())
                elif iData2 == 16: return CyTranslator().getText("TXT_KEY_RANK_START_INFO_ROM_LATE", ())
                elif iData2 == 17: return CyTranslator().getText("TXT_KEY_RANK_START_INFO_HUN", ())
                elif iData2 == 18: return CyTranslator().getText("TXT_KEY_RANK_START_INFO_PERSIA2", ())
            # -------------------------------------
            ...
            # Increase morale (Zeus)
                ...
                if iData2 == 1: return CyTranslator().getText("TXT_KEY_BUTTON_MORALE_UNIT", ())
                else: return CyTranslator().getText("TXT_KEY_BUTTON_BLESS_UNITS", ())
            # Slave -> Latifundium
            elif iData1 == 753:
                if iData2 == 1: return CyTranslator().getText("TXT_KEY_BUTTON_SLAVE2LATIFUNDIUM", ())
                else: return CyTranslator().getText("TXT_KEY_BUTTON_SLAVE2VILLAGE", ())
            # Obsolete units (Tech Screen)
            elif iData1 == 754:
                if bOption: return CyTranslator().getText("TXT_KEY_TECH_OBSOLETES_NO_LINK", (gc.getUnitInfo(iData2).getDescription(),))
                else: return CyTranslator().getText("TXT_KEY_TECH_OBSOLETES_NO_LINK", (gc.getProjectInfo(iData2).getDescription(),))
            ...
            # Statthalter ansiedeln
                ...
                if iData2 == 1: return CyTranslator().getText("TXT_KEY_BUTTON_SETTLE_STATTHALTER2", ()) # General or Hero
                else: return CyTranslator().getText("TXT_KEY_BUTTON_SETTLE_STATTHALTER", ()) # Statthalter
            # Heldendenkmal / Siegesdenkmal aufnehmen / bauen
            elif iData1 == 758:
                if iData2 == 0: return CyTranslator().getText("TXT_KEY_BUTTON_HELDENDENKMAL1", ()) # aufnehmen
                else: return CyTranslator().getText("TXT_KEY_BUTTON_HELDENDENKMAL2", (gc.getBuildingInfo(iData2).getDescription(),)) # bauen
            # Give morale to units
            elif iData1 == 759:
                if iData2 == 2: return CyTranslator().getText("TXT_KEY_BUTTON_SLAVE_DRUIDE", ())
                else: return CyTranslator().getText("TXT_KEY_BUTTON_MORALE_UNITS", ())
    ...
    def doAIMissionReligion(self, pUnit):
        if CvUtil.myRandom(10, "doAIMissionReligion") != 1: return
        
        iPlayer = pUnit.getOwner()
        pPlayer = gc.getPlayer(iPlayer)
        iReligion = self.getUnitReligion(pUnit.getUnitType())
        
        if iReligion == -1: return
        
        if pPlayer.getStateReligion() != iReligion: return
