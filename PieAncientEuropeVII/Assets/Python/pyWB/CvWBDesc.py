...
def filterPlayerIds(list_of_ids, barb_id):
	...
	if barb_id is not None:
		...
		if list_of_ids and list_of_ids[-1] == barb_id:
...
def filterPlayerIds2(list_of_ids, barb_id):
	...
	if barb_id is not None:
		...
		if list_of_ids and list_of_ids[-1][0] == barb_id:
			b = [NUM_CIV_PLAYERS]
...
class CvPlayerDesc:
	...
	## Platy Builder ##
		...
		if pPlayer.getCivilizationType() != CivilizationTypes.NO_CIVILIZATION:
			...
			if civAdjective == defaultAdjective:
...
class CvPlotDesc:
	...
	def read(self, f):
		...
		self.__init__()
		parser = CvWBParser()
		if not parser.findNextToken(f, "BeginPlot"):
...
## Platy Builder ##
	def getAssignedStartingPlots(self):
		...
		if sScenarioName in mapNames:
			...
			MyFile = open("Mods/PieAncientEuropeVII/Assets/XML/Misc/" + mapNames[sScenarioName])
