				...
				for i in xrange(10):
					for j in xrange(7):
						...
						if iSetStorm == 1:
							loopPlot = gc.getMap().plot(pPlot.getX() + i - 1, pPlot.getY() + j - 2)
							if loopPlot is not None and not loopPlot.isNone():
								if loopPlot.getFeatureType() in lIce: continue
	...
	for iHimmelsrichtung in xrange(4):
		...
		for d in xrange(3):
			...
			for i in xrange(10):
				...
				if loopPlot is None or loopPlot.isNone(): break
				elif loopPlot.getFeatureType() == iDarkIce: break
				elif not loopPlot.isWater():
					bDoTsunami = True
...
def doMeteorites():
	...
	if bonusPlotArray:
		iRand = CvUtil.myRandom(8, "doMeteorites4")
		if iRand < 2:
			if iRand == 0:
				iNewBonus = bonus_magnetit
			else:
				iNewBonus = bonus_oreichalkos
			iRandPlot = CvUtil.myRandom(len(bonusPlotArray), "doMeteorites5")
			pRandPlot = bonusPlotArray[iRandPlot]
			pRandPlot.setBonusType(iNewBonus)
			iOwner = pRandPlot.getOwner()
			if iOwner > -1 and gc.getPlayer(iOwner).isHuman():
				CyInterface().addMessage(iOwner, True, 10, CyTranslator().getText("TXT_KEY_NEW_BONUS", (gc.getBonusInfo(iNewBonus).getDescription(), )), None, 2, gc.getBonusInfo(iNewBonus).getButton(), ColorTypes(14), pRandPlot.getX(), pRandPlot.getY(), True, True)
		...
		if pPlot is not None and not pPlot.isNone():
			...
			if pPlot.isWater():
				...
				if not pPlot.isWater() and not pPlot.isPeak() and pPlot.getBonusType(pPlot.getOwner()) == -1 and pPlot.getBonusType(-1) == -1:
					iRand = CvUtil.myRandom(4, "cometBonus")
					if iRand < 2:
						if iRand == 0:
							iNewBonus = bonus_magnetit
						else:
