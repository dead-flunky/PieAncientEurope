...
from CvPythonExtensions import (CyGlobalContext, CyTranslator, CyGame,
                                FontSymbols, CommerceTypes, AttitudeTypes,
                                CyGameTextMgr, MemoryTypes, CyArtFileMgr)
import BugUtil
import FontUtil
import PlayerUtil
import re
...
def getAttitudeCategory(nPlayer, nTarget):
    ...
    # keldath fix - has attitude already checks nPlayer != nTarget
    ...
    if bWarPeace:
        ...
        if pTeam.isAtWar(nTargetTeam):
            ...
            if pTeam.isForcePeace(nTargetTeam):
                ...
                for nOwnerTeam in xrange(gc.getMAX_TEAMS()):
