    ...
    for x in range(iMapW):
      for y in range(iMapH):
        ...
        if loopPlot is not None and not loopPlot.isNone():
          ...
          if iPlotFeature == iDarkIce: continue
                ...
                if iGameTurn > 150 and iGameTurn % 10 == 0:
                  if iPlotOwner == -1:
                    if loopPlot.getNumUnits() == 0 and not loopPlot.isActiveVisible(0):
                      ...
                      if iPlotTerrain == iTerrDesert: Desert.append(loopPlot)
                      elif iPlotTerrain == iTerrTundra: Tundra.append(loopPlot)
                      elif iPlotFeature == iFeatDenseForest: DenseForest.append(loopPlot)
                      elif iPlotFeature == iFeatJungle: Jungle.append(loopPlot)
                      elif iPlotFeature == iFeatForest: Forest.append(loopPlot)
                      elif iPlotTerrain == iTerrPlains: Plains.append(loopPlot)
                      if loopPlot.isHills(): Hills.append(loopPlot)
            # end if - not isCity
...
def setAnimals(eAnimal,plots):
    ...
    # Ausnahmen
    if eAnimal == gc.getInfoTypeForString("UNIT_UR"): iAnimals = 2 - iBarbUnits # maximal 2
    if iAnimals <= 0: return
...
def setUnitIntoCave(pPlot):
    ...
    if len(lUnits):
      ...
      gc.getPlayer(gc.getBARBARIAN_PLAYER()).initUnit(lUnits[iRand], pPlot.getX(), pPlot.getY(), UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
...
def doMoveGrasshoppers(pPlot):
    ...
    # Am Plot bleiben 2:3
    if CvUtil.myRandom(3, "doMoveGrasshoppers") > 0: return
...
def doOlympicGames():
    ...
    if gc.getGame().getProjectCreatedCount(gc.getInfoTypeForString("PROJECT_OLYMPIC_GAMES")) > 0:
      ...
      # alle 4 Runden
      if   gc.getGame().getCalendar() == gc.getInfoTypeForString("CALENDAR_MONTHS"):  iTurns = 48
      elif gc.getGame().getCalendar() == gc.getInfoTypeForString("CALENDAR_SEASONS"): iTurns = 16
      else: iTurns = 4
      ...
      if gc.getGame().getElapsedGameTurns() % iTurns == 1:
        ...
        for iPlayer in range (iNumPlayers):
          ...
          if pPlayer is not None and not pPlayer.isNone() and pPlayer.isAlive(): # and not pPlayer.isBarbarian():
            ...
            if not gc.getTeam(pPlayer.getTeam()).isHasTech(iTechSchaukampf): continue
            
            # Hat der Spieler bereits das Papsttum erforscht, is der Spass vorbei
            if gc.getTeam(pPlayer.getTeam()).isHasTech(iTechPapsttum): continue
            
            # Hat der Spieler Imperialismus erforscht?
            bAllowRomanGods = False
            if gc.getTeam(pPlayer.getTeam()).isHasTech(iTechImperialismus): bAllowRomanGods = True
            ...
            for iCity in range (iNumCities):
              ...
              if not pCity.isNone():
                ...
                if pCity.isHasBuilding(iBuildingOlympionike): pCity.setNumRealBuilding(iBuildingOlympionike,0)
                ...
                if pCity.isHasReligion(iReligionGreek) or bAllowRomanGods and pCity.isHasReligion(iReligionRome):
                  ...
                  if pPlayer.isHuman(): 
                    if iPlayer not in lHumans: lHumans.append(iPlayer)
                    
                  # Liste verschiedener CIVs
                  if iPlayer not in lPlayers: lPlayers.append(iPlayer)
                  
                  # Stadt an den Spielen zulassen
                  lCities4Olympiade.append(pCity)
                  # Verbesserte Chancen:                  
                  if pCity.isHasBuilding(iBuildingStadion):   lCities4Olympiade.append(pCity)
                  if pCity.isHasBuilding(iBuildingGymnasion): lCities4Olympiade.append(pCity)
...
def doCheckFortCulture(pPlot):
    ...
    # Wenn der Plot keinen Besitzer hat
      ...
      for i in range(iNumUnits):
        ...
        if iPlayer not in dictPlayers: dictPlayers[iPlayer] = 1
        else: dictPlayers[iPlayer] += 1
